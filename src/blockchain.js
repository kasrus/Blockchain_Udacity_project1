...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            var timeDifference = currentTime - timeSent;
            ...
            if(timeDifference < 300) {
                ...
                if(isValid) {
                    ...
                    //add the block into the chain
                    let resolvedBlock = await self._addBlock(newBlock);
                    this.validateChain();
                    resolve(resolvedBlock);
                } else {
                    resolve(new Error("Cannot add the block to the chain"));
                }
                
            } else {
                resolve(new Error("Took more than 5 minutes!"));
            }
        });
    }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
            let block = this.chain.filter(function(block){
                return block.hash === hash;
            });
            if(block) {
                resolve(block);
            } else {
                reject(new Error("Can't find a block with that hash"));
            }
        });
    }
